...
<div>
  ...
  {#if !$isEnterprise}
    <div class="rounded-xl border border-amber-200 bg-amber-50 p-6 text-center">
      <p class="text-sm font-medium text-amber-800">Audit log is available on the Enterprise plan.</p>
          ...
          <tbody class="divide-y divide-gray-100">
            {#each entries as entry}
              <tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-3">
                  <span class="rounded-full px-2 py-0.5 text-xs font-medium {actionColor[entry.action] ?? 'bg-gray-100 text-gray-600'}">
                    {entry.action}
                  </span>
                </td>
                <td class="px-4 py-3 capitalize text-gray-700">{entry.entity_type}</td>
                <td class="px-4 py-3 font-mono text-xs text-gray-500">{entry.entity_id.slice(0, 12)}…</td>
              </tr>
            {/each}
          </tbody>
        </table>
